...
func (w DownloadLiveVideoWorker) Work(ctx context.Context, job *river.Job[DownloadLiveVideoArgs]) error {
	...
	streamEndedForNotification := downloadErr == nil
	...
	if downloadErr != nil {
		if errors.Is(downloadErr, context.Canceled) {
			...
			if platformSvc, platformErr := PlatformFromContext(ctx); platformErr == nil {
				probeCtx, cancelProbe := context.WithTimeout(context.WithoutCancel(ctx), 20*time.Second)
				isLive, liveErr := platformSvc.CheckIfStreamIsLive(probeCtx, dbItems.Channel.Name)
				cancelProbe()
				if liveErr != nil {
