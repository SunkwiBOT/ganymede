...
// RecoverInterruptedLiveArchives runs a recovery pass before the worker starts
// accepting periodic live checks. This finalizes a crash-left segment before a
// continuation recording can be created for a streamer who is still live.
func RecoverInterruptedLiveArchives(ctx context.Context, riverClient *river.Client[pgx.Tx]) error {
...
func archiveJobProgress(job *rivertype.JobRow) (time.Time, RiverJobArgs, error) {
	...
	heartbeat := args.Input.HeartBeatTime
	if heartbeat.IsZero() {
		if job.AttemptedAt != nil {
			heartbeat = *job.AttemptedAt
		} else {
			heartbeat = job.CreatedAt
		}
	}
	return heartbeat, args, nil
}
...
func recoverOrphanedLiveVideoArchives(ctx context.Context, store *database.Database, riverClient *river.Client[pgx.Tx]) error {
	...
	for _, q := range stuckQueues {
		...
		if q.TaskVideoDownload == utils.Pending {
			if _, err := riverClient.Insert(ctx, &DownloadLiveVideoArgs{
				Continue: true,
...
func cleanupEmptyFailedLiveArchives(ctx context.Context, store *database.Database, riverClient *river.Client[pgx.Tx]) error {
	...
	for _, q := range candidates {
		...
		validationErr := validateRecoverableLiveVideoInput(&dbItems.Video)
...
func validateRecoverableLiveVideoInput(video *ent.Vod) error {
	...
	if video.TmpVideoConvertPath != "" && utils.FileExists(video.TmpVideoConvertPath) {
		if nonEmptyFileExists(video.TmpVideoConvertPath) || nonEmptyFileExists(video.TmpVideoDownloadPath) {
			return nil
		}
