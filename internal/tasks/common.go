...
func (w CreateDirectoryWorker) Work(ctx context.Context, job *river.Job[CreateDirectoryArgs]) error {
	...
	if job.Args.Continue {
		if dbItems.Queue.LiveArchive {
			next = append(next,
				transactionalJob{Args: &SaveVideoInfoArgs{Continue: false, Input: nextArchiveInput(job.Args.Input)}},
				transactionalJob{Args: &DownloadThumbnailArgs{Continue: false, Input: nextArchiveInput(job.Args.Input)}},
				transactionalJob{Args: &DownloadLiveVideoArgs{Continue: true, Input: nextArchiveInput(job.Args.Input)}},
			)

			hasLive, err := store.Client.Channel.Query().
				Where(entChannel.ID(dbItems.Channel.ID)).
				QueryLive().
				Exist(ctx)
			if err != nil {
				return err
			...
			if hasLive {
				...
				if watchedChannel.UpdateMetadataMinutes > 0 {
					next = append(next, transactionalJob{
						Args: &UpdateLiveStreamMetadataArgs{
							Continue: false,
